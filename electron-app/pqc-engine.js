...
const PQCEngine = {
    ...
    kemEncapsulate(publicKey) {
        const pk = typeof publicKey === 'string'
    ...
    kemDecapsulate(cipherText, secretKey) {
        const ct = typeof cipherText === 'string'
            ? Uint8Array.from(Buffer.from(cipherText, 'hex'))
            : cipherText;
    ...
    runSelfTest() {
        ...
        try {
            ...
            if (!t1) allPass = false;
            lines.push(`[1/6] ML-KEM-768 keygen ....... ${t1 ? '✅ PASS' : '❌ FAIL'}  pk=${kg1.publicKeyBytes}B  sk=${kg1.secretKeyBytes}B  (${kg1.ms}ms)`);
            ...
            if (!t2) allPass = false;
            lines.push(`[2/6] ML-KEM-768 encapsulate .. ${t2 ? '✅ PASS' : '❌ FAIL'}  ct=${enc.cipherTextBytes}B  ss=${enc.sharedSecretBytes}B  (${enc.ms}ms)`);
            ...
            if (!t3) allPass = false;
            lines.push(`[3/6] ML-KEM-768 decapsulate .. ${t3 ? '✅ PASS' : '❌ FAIL'}  secrets-match=${t3}  (${dec.ms}ms)`);
            ...
            if (!t4) allPass = false;
            lines.push(`[4/6] ML-KEM-768 wrong-key .... ${t4 ? '✅ PASS' : '❌ FAIL'}  implicit-rejection=${t4}  (${decBad.ms}ms)`);

            // 5 — ML-DSA-65 Keygen
            const dsaKg = this.dsaKeygen();
            const t5 = dsaKg.publicKeyBytes === 1952 && dsaKg.secretKeyBytes > 0;
            if (!t5) allPass = false;
            lines.push(`[5/6] ML-DSA-65  keygen ....... ${t5 ? '✅ PASS' : '❌ FAIL'}  pk=${dsaKg.publicKeyBytes}B  sk=${dsaKg.secretKeyBytes}B  (${dsaKg.ms}ms)`);

            // 6 — ML-DSA-65 Key structure
            const t6 = dsaKg.publicKey instanceof Uint8Array && dsaKg.publicKey.length === 1952;
            if (!t6) allPass = false;
            lines.push(`[6/6] ML-DSA-65  key format .... ${t6 ? '✅ PASS' : '❌ FAIL'}  Uint8Array(${dsaKg.publicKey.length})  (${dsaKg.ms}ms)`);
        ...
        if (allPass) {
            logSession({ domain: 'localhost (self-test)', kem: 'ML-KEM-768', sig: 'ML-DSA-65', status: 'COMPLETED', ca: 'KryptonBrowser', ms: 0 });
        }
    ...
    recordTlsSession(domain) {
        ...
        try {
            ...
            logSession({ domain, kem: 'ML-KEM-768', sig: 'ML-DSA-65', status: verified ? 'COMPLETED' : 'FAILED', ca: '—', ms });
            return { ms, verified };
        } catch (e) {
