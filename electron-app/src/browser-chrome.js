...
import { sanitizeHTML, getGreeting, getTimeString, pick, formatBytes, formatSpeed, getFileIcon } from './js/utils.js';
...
// Built-in extensions
const extensions = [
    { id: 'pqc-shield', name: 'PQC Shield', desc: 'Quantum-safe connection monitor', icon: '🛡️', color: 'rgba(251,146,60,0.15)', enabled: true },
    { id: 'ad-guard', name: 'KryptonShield', desc: 'Block ads, trackers & fingerprinting', icon: '🚫', color: 'rgba(248,113,113,0.15)', enabled: true },
    { id: 'dark-reader', name: 'Dark Reader', desc: 'Dark mode for every website', icon: '🌙', color: 'rgba(96,165,250,0.15)', enabled: true },
    { id: 'password-mgr', name: 'Password Manager', desc: 'Auto-fill passwords securely', icon: '🔑', color: 'rgba(52,211,153,0.15)', enabled: false },
    { id: 'speedreader', name: 'Speedreader', desc: 'Declutter pages for focused reading', icon: '📖', color: 'rgba(167,139,250,0.15)', enabled: true },
    { id: 'translate', name: 'Translate', desc: 'Private in-browser translation', icon: '🌐', color: 'rgba(34,211,238,0.15)', enabled: true },
];
...
const QUOTES = [
    ...
    { text: 'Encryption works. Properly implemented strong crypto systems are one of the few things you can rely on.', author: 'Edward Snowden' },
];
...
function recordHistory(url, title, favicon, timestamp) {
    ...
    try {
        let hist = JSON.parse(localStorage.getItem('krypton_history') || '[]');
        hist.unshift({ url, title: title || url, favicon: favicon || null, timestamp: timestamp || Date.now() });
...
function createTab(url = 'krypton://newtab') {
    ...
    if (isNewTab) { faviconIcon = 'security'; initialTitle = 'New Tab'; }
    else if (isHistory) { faviconIcon = 'history'; initialTitle = 'History'; }
    else if (isSettings) { faviconIcon = 'settings'; initialTitle = 'Settings'; }
    else if (isExtensions) { faviconIcon = 'extension'; initialTitle = 'Extensions'; }
    else if (url === 'krypton://pqc-security') { faviconIcon = 'shield'; }
...
function createNewTabPage(tabId) {
  ...
  `;
    ...
    startClock(tabId);
    div.querySelector(`#ntp-search-${tabId}`).addEventListener('click', () => { $urlInput.focus(); $urlInput.select(); });
    div.querySelector(`#ntp-pqc-${tabId}`).addEventListener('click', () => createTab('krypton://pqc-security'));
    div.querySelector(`#ntp-history-${tabId}`).addEventListener('click', () => createTab('krypton://history'));

    // Set quote text via JS
    const quote = pick(QUOTES);
    const quoteEl = div.querySelector(`#ntp-quote-${tabId}`);
    if (quoteEl) quoteEl.innerHTML = `"${quote.text}"<span class="ntp-quote-author">— ${quote.author}</span>`;
    ...
    function refreshNtpStats() {
        ...
        window.kryptonBrowser.getBlockingStats().then(stats => {
            if (!stats) return;
    ...
    const ntpStatsInterval = setInterval(() => {
        ...
        if (!div.isConnected) { clearInterval(ntpStatsInterval); return; }
        refreshNtpStats();
    }, 5000);
...
function createWebview(tabId, url) {
    ...
    wv.addEventListener('did-start-loading', () => { showLoading(true); updateReloadButton(true); });
    wv.addEventListener('did-stop-loading', () => { showLoading(false); updateReloadButton(false); });
    ...
    wv.addEventListener('did-navigate-in-page', (e) => {
        const tab = getTab(tabId);
        if (tab && tabId === activeTabId) { tab.url = e.url; $urlInput.value = e.url; }
    });
    ...
    wv.addEventListener('did-fail-load', (e) => { if (e.errorCode !== -3) showLoading(false); });
    ...
    setTimeout(() => { if (typeof hookFindInPage === 'function') hookFindInPage(wv); }, 0);
...
function navigateInternalPage(tabId, url) {
    ...
    if (tab.newTabPage) { tab.newTabPage.remove(); tab.newTabPage = null; tab.isNewTab = false; }
    if (tab.inAppPage) { tab.inAppPage.remove(); tab.inAppPage = null; tab.isInAppPage = false; }
    ...
    if (tabId === activeTabId) { showActiveContent(tab); $urlInput.value = url; updateSecurityIndicator(url); }
}
...
function createHistoryPage(tabId) {
    ...
    let hist = [];
    try { hist = JSON.parse(localStorage.getItem('krypton_history') || '[]'); } catch (e) { }
    ...
    function render(filter) {
        ...
        const filtered = f
            ? hist.filter(h => (h.title || '').toLowerCase().includes(f) || (h.url || '').toLowerCase().includes(f))
            : hist;

        if (filtered.length === 0) {
            container.innerHTML = '<div class="hist-empty"><span class="material-icons-outlined">history</span><p>' +
                (f ? 'No matching history' : 'No browsing history yet.') + '</p></div>';
            return;
        }
        ...
        for (let i = 0; i < filtered.length; i++) {
            ...
            const dayStr = ds.getTime() === today.getTime()
                ? 'Today – ' + d.toLocaleDateString(undefined, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
                : ds.getTime() === yday.getTime()
                    ? 'Yesterday – ' + d.toLocaleDateString(undefined, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
            ...
            html += '<div class="hist-row" data-url="' + h.url.replace(/"/g, '&quot;') + '" data-ts="' + h.timestamp + '">' +
                '<input type="checkbox" class="hist-check">' +
                '<span class="hist-time">' + time + '</span>' +
                faviconHtml +
                '<span class="hist-title">' + (h.title || h.url) + '</span>' +
                '<span class="hist-domain">' + domain + '</span>' +
                '<button class="hist-more" data-ts="' + h.timestamp + '" title="Delete this entry"><span class="material-icons-outlined">delete</span></button>' +
                '</div>';
        }
        if (lastDay) html += '</div>';
    ...
    div.innerHTML =
        ...
        '<a class="hist-nav-item hist-delete-link" id="hc-' + tabId + '"><span class="material-icons-outlined">delete_outline</span>Delete browsing data</a>' +
        '</div>' +
        '<div class="hist-main">' +
        '<div class="hist-toolbar">' +
        '<div class="hist-search"><span class="material-icons-outlined">search</span>' +
        '<input type="text" placeholder="Search history" id="hs-' + tabId + '">' +
        '</div>' +
        '</div>' +
    ...
    div.addEventListener('click', (e) => {
        ...
        if (row) { navigateActiveTab(row.dataset.url); return; }
    });

    const si = div.querySelector('#hs-' + tabId);
    if (si) si.addEventListener('input', () => render(si.value));
    const cb = div.querySelector('#hc-' + tabId);
    if (cb) cb.addEventListener('click', () => { hist = []; localStorage.setItem('krypton_history', '[]'); render(''); });

    // Expose a refresh function so it re-reads localStorage when the tab is re-activated
    div._refreshHistory = function () {
        try { hist = JSON.parse(localStorage.getItem('krypton_history') || '[]'); } catch (e) { hist = []; }
        const si = div.querySelector('#hs-' + tabId);
        render(si ? si.value : '');
    };
...
function createSettingsPage(tabId) {
    ...
    ACCENT_COLORS.forEach(c => {
        ...
        colorSwatchesHtml += '<div class="settings-color-swatch' + sel + '" data-color="' + c.value + '" title="' + c.name + '" style="background:' + c.value + ';color:' + c.value + '"></div>';
    });
    ...
    Object.keys(SEARCH_ENGINES).forEach(key => {
        ...
        searchHtml += '<label class="settings-radio-item"><input type="radio" name="se-' + tabId + '" value="' + key + '"' + checked + '><span class="settings-radio-label">' + se.name + '</span></label>';
    });
    ...
    div.innerHTML =
        ...
        '</div>' +
        '<div class="hist-main">' +
        '<div class="settings-content" id="settings-content-' + tabId + '">' +
        ...
        '<div class="settings-toggle' + (showBookmarks ? ' active' : '') + '" id="stgl-bookmarks-' + tabId + '"></div></div>' +
        ...
        '<div class="settings-toggle' + (showWallpaper ? ' active' : '') + '" id="stgl-wallpaper-' + tabId + '"></div></div>' +
        ...
        '<select class="settings-select" id="stgl-fontsize-' + tabId + '"><option value="small"' + (fontSize === 'small' ? ' selected' : '') + '>Small</option><option value="medium"' + (fontSize === 'medium' ? ' selected' : '') + '>Medium</option><option value="large"' + (fontSize === 'large' ? ' selected' : '') + '>Large</option></select></div>' +
        '</div>' +
        '<div class="settings-card">' +
        ...
        '<div class="settings-color-picker" id="stgl-color-' + tabId + '">' + colorSwatchesHtml + '</div>' +
        '</div>' +
        '</div>' +
        ...
        '<div class="settings-toggle active disabled" id="stgl-pqc-' + tabId + '" title="PQC encryption is always enabled"></div></div>' +
        ...
        '<div class="settings-toggle' + (blockCookies ? ' active' : '') + '" id="stgl-cookies-' + tabId + '"></div></div>' +
        ...
        '<div class="settings-toggle' + (sendDNT ? ' active' : '') + '" id="stgl-dnt-' + tabId + '"></div></div>' +
        '</div>' +
        '<div class="settings-card">' +
        ...
        '<button class="settings-btn-danger" id="settings-clear-data-' + tabId + '">Clear data…</button></div>' +
        '</div>' +
        '</div>' +
        ...
        '<label class="settings-radio-item"><input type="radio" name="su-' + tabId + '" value="newtab"' + (startupMode === 'newtab' ? ' checked' : '') + '><div><span class="settings-radio-label">Open the New Tab page</span><div class="settings-radio-desc">Start with a fresh new tab page</div></div></label>' +
        '<label class="settings-radio-item"><input type="radio" name="su-' + tabId + '" value="last-session"' + (startupMode === 'last-session' ? ' checked' : '') + '><div><span class="settings-radio-label">Continue where you left off</span><div class="settings-radio-desc">Restore your last browsing session</div></div></label>' +
        '<label class="settings-radio-item"><input type="radio" name="su-' + tabId + '" value="custom"' + (startupMode === 'custom' ? ' checked' : '') + '><div><span class="settings-radio-label">Open a specific page</span><div class="settings-radio-desc">Enter a URL to open on startup</div></div></label>' +
        '</div>' +
        '<div style="padding:8px 12px 4px"><input type="text" class="settings-input" id="stgl-startup-url-' + tabId + '" placeholder="https://example.com" value="' + startupUrl.replace(/"/g, '&quot;') + '"' + (startupMode !== 'custom' ? ' disabled' : '') + '></div>' +
        '</div>' +
        '</div>' +
        ...
        '<div class="settings-toggle' + (askDownloadLoc ? ' active' : '') + '" id="stgl-download-' + tabId + '"></div></div>' +
        '</div>' +
        '</div>' +
        ...
        '<button class="settings-btn-danger" id="settings-print-page-' + tabId + '" style="border-color:var(--accent);color:var(--accent)">Print…</button></div>' +
        '</div>' +
        '<div class="settings-card">' +
        ...
        '<div class="settings-toggle' + (localStorage.getItem('krypton_print_headers') !== 'false' ? ' active' : '') + '" id="stgl-print-headers-' + tabId + '"></div></div>' +
        ...
        '<div class="settings-toggle' + (localStorage.getItem('krypton_print_bg') === 'true' ? ' active' : '') + '" id="stgl-print-bg-' + tabId + '"></div></div>' +
        '</div>' +
        '</div>' +
        ...
        '<div class="settings-about-engine">Chromium ' + (window.kryptonBrowser ? window.kryptonBrowser.chromeVersion : '') + ' · Electron ' + (window.kryptonBrowser ? window.kryptonBrowser.electronVersion : '') + '</div>' +
        '</div>' +
        '</div>' +
    ...
    div.querySelectorAll('[data-settings-section]').forEach(item => {
        item.addEventListener('click', () => {
            ...
            div.querySelectorAll('.settings-section').forEach(s => { s.classList.remove('active'); s.style.display = ''; });
    ...
    const MAIN_PROCESS_KEYS = new Set([
        'krypton_block_cookies', 'krypton_send_dnt', 'krypton_ad_block',
        'krypton_https_upgrade', 'krypton_ask_download_loc',
    ]);
    ...
    if (colorPicker) {
        colorPicker.addEventListener('click', (e) => {
            ...
            if (!swatch) return;
            colorPicker.querySelectorAll('.settings-color-swatch').forEach(s => s.classList.remove('selected'));
    ...
    function renderExtList() {
        ...
        extensions.forEach((ext, i) => {
            html +=
                ...
                '<div style="width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;background:' + ext.color + ';flex-shrink:0">' + ext.icon + '</div>' +
                '<div style="flex:1;min-width:0">' +
                '<div style="font-size:14px;font-weight:500;color:var(--text-primary)">' + ext.name + '</div>' +
                '<div style="font-size:12px;color:var(--text-muted);margin-top:2px">' + ext.desc + '</div>' +
                '</div>' +
                '<div class="ext-page-toggle" data-ext-index="' + i + '" style="width:40px;height:22px;border-radius:11px;position:relative;cursor:pointer;flex-shrink:0;background:' + (ext.enabled ? 'var(--accent)' : 'var(--border)') + '">' +
                '<div style="position:absolute;' + (ext.enabled ? 'right:2px' : 'left:2px') + ';top:2px;width:18px;height:18px;border-radius:50%;background:#fff;transition:all 0.2s"></div>' +
                '</div>' +
                '</div>';
        });
...
function navigateActiveTab(input) {
    ...
    if (url.startsWith('krypton://')) {
        ...
        if (url === 'krypton://history' || url === 'krypton://settings' || url === 'krypton://extensions') {
            createTab(url);
            return;
        }
    ...
    if (tab.isNewTab) {
        if (tab.newTabPage) { tab.newTabPage.remove(); tab.newTabPage = null; }
        tab.isNewTab = false;
    }
    if (tab.isInAppPage) {
        if (tab.inAppPage) { tab.inAppPage.remove(); tab.inAppPage = null; }
    ...
    tab.tabEl.querySelector('.tab-favicon').innerHTML = '<span class="material-icons-outlined">language</span>';
    $urlInput.value = url;
...
function updateNavButtons() {
    ...
    if (tab && tab.webview) {
        try { $back.disabled = !tab.webview.canGoBack(); $forward.disabled = !tab.webview.canGoForward(); }
        catch (e) { $back.disabled = true; $forward.disabled = true; }
    } else { $back.disabled = true; $forward.disabled = true; }
}

function updateReloadButton(loading) { $reloadIcon.textContent = loading ? 'close' : 'refresh'; }

function showLoading(isLoading) {
    if (isLoading) $loadingBar.classList.add('active');
    else { $loadingBar.classList.remove('active'); $loadingProgress.style.width = '0'; }
}
...
async function updateShieldsPanel() {
    ...
    if (tab && tab.url && !tab.url.startsWith('krypton://')) {
        ...
        } catch { $site.textContent = tab.url; }
    } else {
    ...
    const [stats, siteStats] = await Promise.all([
        ...
        pageUrl ? window.kryptonBrowser.getSiteBlockCount(pageUrl).catch(() => null) : Promise.resolve(null),
    ]);
    ...
    const adBlock = await window.kryptonBrowser.getConfig('krypton_ad_block', 'true').catch(() => 'true');
    const httpsUp = await window.kryptonBrowser.getConfig('krypton_https_upgrade', 'true').catch(() => 'true');
...
(function wireShieldsControls() {
    ...
    if (globalToggle) {
        globalToggle.addEventListener('click', () => {
            ...
            showSettingsToast(isOn ? 'KryptonShield enabled for this site' : 'KryptonShield disabled for this site');
        });
    }
    ...
    if (httpsToggle) {
        httpsToggle.addEventListener('click', () => {
            ...
            if (window.kryptonBrowser) window.kryptonBrowser.setConfig('krypton_https_upgrade', isOn ? 'true' : 'false');
    ...
    if (scriptsToggle) {
        scriptsToggle.addEventListener('click', () => {
            ...
            showSettingsToast(scriptsToggle.classList.contains('active') ? 'Script blocking enabled' : 'Script blocking disabled');
        });
    }

    const advBtn = document.getElementById('shields-advanced-btn');
    if (advBtn) advBtn.addEventListener('click', () => { $shieldsPanel.style.display = 'none'; createTab('krypton://settings'); });

    const reportBtn = document.getElementById('shields-report-btn');
    if (reportBtn) reportBtn.addEventListener('click', () => { showSettingsToast('Broken site report submitted'); $shieldsPanel.style.display = 'none'; });
})();
...
$tabCtxMenu.addEventListener('click', (e) => {
    ...
    if (!tab) { hideTabContextMenu(); return; }
    ...
    switch (action) {
        ...
        case 'mute':
            tab.muted = !tab.muted;
            if (tab.webview) {
                try { tab.webview.setAudioMuted(tab.muted); } catch (e) { }
            }
            break;
        case 'reload':
            ...
            break;
        case 'close-others':
            const othersToClose = tabs.filter(t => t.id !== tabCtxTargetId && !t.pinned).map(t => t.id);
            othersToClose.forEach(id => closeTab(id));
            break;
        case 'close-right': {
            const idx = tabs.findIndex(t => t.id === tabCtxTargetId);
            const rightToClose = tabs.slice(idx + 1).filter(t => !t.pinned).map(t => t.id);
...
function renderSidebarBookmarks(filter = '') {
    ...
    list.innerHTML = '';
    const f = filter.toLowerCase();
    const filtered = f ? bookmarks.filter(b => b.title.toLowerCase().includes(f) || b.url.toLowerCase().includes(f)) : bookmarks;

    if (filtered.length === 0) {
        list.innerHTML = '<div class="sidebar-empty"><span class="material-icons-outlined">bookmark_border</span><p>' + (f ? 'No matching bookmarks' : 'No bookmarks yet') + '</p></div>';
        return;
    }
...
function renderSidebarHistory(filter = '') {
    ...
    const f = filter.toLowerCase();
    const filtered = f ? hist.filter(h => (h.title || '').toLowerCase().includes(f) || (h.url || '').toLowerCase().includes(f)) : hist;
    const limited = filtered.slice(0, 50);

    if (limited.length === 0) {
        list.innerHTML = '<div class="sidebar-empty"><span class="material-icons-outlined">history</span><p>' + (f ? 'No matching history' : 'No history yet') + '</p></div>';
        return;
    }
...
if (sidebarBookmarkSearch) sidebarBookmarkSearch.addEventListener('input', () => renderSidebarBookmarks(sidebarBookmarkSearch.value));

const sidebarHistorySearch = document.getElementById('sidebar-history-search');
if (sidebarHistorySearch) sidebarHistorySearch.addEventListener('input', () => renderSidebarHistory(sidebarHistorySearch.value));

// Sidebar add bookmark
const sidebarAddBookmark = document.getElementById('sidebar-add-bookmark');
if (sidebarAddBookmark) sidebarAddBookmark.addEventListener('click', () => { toggleBookmark(); renderSidebarBookmarks(); });

// Sidebar add to reading list
const sidebarAddReading = document.getElementById('sidebar-add-reading');
if (sidebarAddReading) sidebarAddReading.addEventListener('click', () => {
    const tab = getActiveTab();
    if (tab && tab.url && !tab.url.startsWith('krypton://')) {
...
if (sidebarClearHistory) sidebarClearHistory.addEventListener('click', () => {
...
function activateReaderMode() {
    ...
    `).then(result => {
        ...
        $readerContent.innerHTML = '<h1>Reader Mode</h1><p>Could not extract content from this page. Try a different page with article content.</p>';
    });
...
function togglePrivateMode() {
    ...
    showSettingsToast(isPrivateMode ? 'Private browsing: no history will be saved' : 'Exited private mode');
}
...
function handleMenuAction(action) {
    ...
    switch (action) {
        ...
        case 'new-window': createTab(); break;
        case 'private-window': togglePrivateMode(); createTab(); break;
        case 'pqc-panel': createTab('krypton://pqc-security'); break;
        case 'history': createTab('krypton://history'); break;
        case 'bookmarks': toggleSidebar(); break;
        case 'downloads': toggleDownloadsPanel(); break;
        case 'extensions': toggleExtensionsPanel(); break;
        ...
        case 'shield-toggle': toggleAdBlocking(); break;
        case 'settings': createTab('krypton://settings'); break;
    }
}
...
function renderDownloadItem(info) {
    ...
    if (isDone) {
        ...
        if (btnOpen) btnOpen.addEventListener('click', () => window.kryptonBrowser.openDownload(info.savePath));
        if (btnFolder) btnFolder.addEventListener('click', () => window.kryptonBrowser.showDownloadInFolder(info.savePath));
    }
}
...
async function updateSecurityPopup() {
    ...
    if (tab.url && tab.url.startsWith('https://')) {
        ...
        if (secInfo && secInfo.secure) {
            ...
            $subtitle.textContent = secInfo.pqcActive ? 'Post-quantum encryption active' : 'Standard encryption active';
            $popupLock.textContent = 'lock'; $popupLock.style.color = 'var(--accent-green)';
            ...
            if (secInfo.pqcActive) {
                $pqc.textContent = '✓ Active'; $pqc.className = 'popup-value pqc-active';
            } else {
                $pqc.textContent = '⚠ Inactive'; $pqc.className = 'popup-value'; $pqc.style.color = 'var(--text-muted)';
            }
        } else {
            $title.textContent = 'Connection is secure';
            $subtitle.textContent = 'Standard encryption active';
            $popupLock.textContent = 'lock'; $popupLock.style.color = 'var(--accent-green)';
            $protocol.textContent = 'TLS 1.2/1.3'; $kem.textContent = 'Standard';
        ...
        $popupLock.textContent = 'lock_open'; $popupLock.style.color = 'var(--accent-red)';
        $protocol.textContent = 'None'; $kem.textContent = 'None';
        $cipher.textContent = 'None';
        $pqc.textContent = '✗ Inactive'; $pqc.className = 'popup-value'; $pqc.style.color = 'var(--accent-red)';
    } else {
        $title.textContent = 'Internal page';
        $subtitle.textContent = 'KryptonBrowser internal resources';
        $popupLock.textContent = 'shield'; $popupLock.style.color = 'var(--accent-green)';
        $protocol.textContent = 'Local'; $kem.textContent = 'N/A';
        $cipher.textContent = 'N/A';
        $pqc.textContent = 'N/A'; $pqc.className = 'popup-value'; $pqc.style.color = 'var(--text-muted)';
    }
}
...
function showAutocomplete(query) {
    ...
    results.push({ title: 'Search ' + _curSE.name + ' for "' + query + '"', url: _curSE.url + encodeURIComponent(query), icon: 'search', type: 'Search' });
    ...
    limited.forEach((item, i) => {
        ...
        if (matchIdx >= 0) {
            displayTitle = displayTitle.substring(0, matchIdx) +
                '<b>' + displayTitle.substring(matchIdx, matchIdx + q.length) + '</b>' +
                displayTitle.substring(matchIdx + q.length);
        }

        row.innerHTML = sanitizeHTML(
            '<span class="material-icons-outlined">' + item.icon + '</span>' +
            '<span class="ac-title">' + displayTitle + '</span>' +
            '<span class="ac-url">' + item.url + '</span>' +
...
$urlInput.addEventListener('keydown', (e) => {
    if ($acDropdown.style.display !== 'none') {
        if (e.key === 'ArrowDown') { e.preventDefault(); selectAcItem(1); return; }
        if (e.key === 'ArrowUp') { e.preventDefault(); selectAcItem(-1); return; }
    }
    if (e.key === 'Enter') {
        e.preventDefault();
        hideAutocomplete();
...
// Zoom controls
document.getElementById('zoom-out').addEventListener('click', (e) => { e.stopPropagation(); setZoom(-10); });
document.getElementById('zoom-in').addEventListener('click', (e) => { e.stopPropagation(); setZoom(10); });
...
document.addEventListener('click', (e) => {
    ...
    const shieldsPanel = document.getElementById('shields-panel');
    if (shieldsPanel && shieldsPanel.style.display === 'flex' &&
        !shieldsPanel.contains(e.target) && !e.target.closest('#btn-shield')) {
        shieldsPanel.style.display = 'none';
    }
    // Close sidebar on outside click (only if click is not in sidebar or its toggle)
    const sidebar = document.getElementById('sidebar');
    if (sidebar && sidebarOpen &&
        !sidebar.contains(e.target) && !e.target.closest('#btn-sidebar')) {
        toggleSidebar();
    }
});
...
function doFind(forward = true) {
    ...
    const tab = getActiveTab();
    if (!tab || !tab.webview || !query) { $findCount.textContent = ''; return; }
...
$findInput.addEventListener('input', () => {
    const query = $findInput.value;
    const tab = getActiveTab();
    if (!query) { $findCount.textContent = ''; $findInput.classList.remove('no-match'); return; }
...
function showContextMenu(params, x, y, wv) {
    ...
    if (params.linkURL) {
        items.push({ icon: 'open_in_new', label: 'Open Link in New Tab', action: () => createTab(params.linkURL) });
        items.push({ icon: 'content_copy', label: 'Copy Link Address', action: () => navigator.clipboard.writeText(params.linkURL) });
        items.push('---');
    }

    if (params.mediaType === 'image') {
        items.push({ icon: 'image', label: 'Copy Image URL', action: () => navigator.clipboard.writeText(params.srcURL) });
        items.push({ icon: 'download', label: 'Save Image As…', action: () => { if (wv) wv.downloadURL(params.srcURL); } });
        items.push('---');
    }

    if (params.selectionText) {
        items.push({ icon: 'content_copy', label: 'Copy', action: () => navigator.clipboard.writeText(params.selectionText) });
        const q = encodeURIComponent(params.selectionText);
        items.push({ icon: 'search', label: `Search for “${params.selectionText.substring(0, 30)}…”`, action: () => createTab(getSearchUrl(params.selectionText)) });
        items.push('---');
    }

    if (!params.linkURL && !params.selectionText) {
        items.push({ icon: 'arrow_back', label: 'Back', action: () => { const t = getActiveTab(); if (t && t.webview) t.webview.goBack(); } });
        items.push({ icon: 'refresh', label: 'Reload', action: () => { const t = getActiveTab(); if (t && t.webview) t.webview.reload(); } });
        items.push({ icon: 'bookmark_border', label: 'Bookmark This Page', action: toggleBookmark });
        items.push('---');
        items.push({ icon: 'code', label: 'View Page Source', action: () => { const t = getActiveTab(); if (t && t.url) createTab('view-source:' + t.url); } });
    }

    items.push({ icon: 'bug_report', label: 'Inspect Element', action: () => { if (wv) { try { wv.openDevTools({ mode: 'detach' }); } catch (e) { } } } });
...
const PERM_INFO = {
    ...
    geolocation: { icon: 'location_on', label: 'Location Access', desc: 'wants to know your location.' },
    notifications: { icon: 'notifications', label: 'Notifications', desc: 'wants to send you notifications.' },
    midi: { icon: 'piano', label: 'MIDI Device', desc: 'wants to access MIDI devices.' },
};

function showPermissionDialog(details) {
    const info = PERM_INFO[details.permission] || { icon: 'security', label: 'Permission Request', desc: 'is requesting a permission.' };
    let origin = details.origin || 'A website';
    try { origin = new URL(details.origin).hostname; } catch (e) { }

    $permOverlay.querySelector('#perm-icon').innerHTML = `<span class="material-icons-outlined">${info.icon}</span>`;
    $permOverlay.querySelector('#perm-title').textContent = info.label;
    $permOverlay.querySelector('#perm-origin').textContent = origin;
    $permOverlay.querySelector('#perm-desc').textContent = `${origin} ${info.desc} KryptonBrowser has blocked this request for security.`;
    $permOverlay.style.display = 'flex';

    document.getElementById('perm-deny').onclick = () => { $permOverlay.style.display = 'none'; };
    document.getElementById('perm-allow').onclick = () => { $permOverlay.style.display = 'none'; showSettingsToast('Note: permission enforcement is managed by the system'); };
}
...
// Sync on startup
['krypton_ad_block', 'krypton_https_upgrade', 'krypton_send_dnt', 'krypton_block_cookies'].forEach(k => syncSettingToMain(k));


// Patch settings page wiring via localStorage event (settings page writes to localStorage, we relay to main)
window.addEventListener('storage', (e) => {
    const syncKeys = ['krypton_ad_block', 'krypton_https_upgrade', 'krypton_send_dnt', 'krypton_block_cookies'];
...
if (window.kryptonBrowser) {
    ...
    window.kryptonBrowser.onCloseTab(() => { if (activeTabId) closeTab(activeTabId); });
    window.kryptonBrowser.onFocusUrlBar(() => { $urlInput.focus(); $urlInput.select(); });
    ...
    if (window.kryptonBrowser.onMenuAction) {
        window.kryptonBrowser.onMenuAction((action) => {
            switch (action) {
                case 'private-window': togglePrivateMode(); break;
                case 'toggle-sidebar': toggleSidebar(); break;
                case 'reader-mode': toggleReaderMode(); break;
                case 'toggle-shields': toggleShieldsPanel(); break;
            }
        });
    }
}
...
document.getElementById('dl-panel-close').addEventListener('click', () => { $downloadsPanel.style.display = 'none'; });
...
(function initStartup() {
    ...
    if (mode === 'last-session') {
        ...
        if (customUrl && customUrl.trim()) {
            createTab(customUrl.trim().startsWith('http') ? customUrl.trim() : 'https://' + customUrl.trim());
        } else {
            createTab('krypton://newtab');
        }
    } else {
