...
function buildNoiseScript(level) {
  ...
  try{
    ...
    HTMLCanvasElement.prototype.toDataURL=function(...a){
      ...
      if(ctx){const d=ctx.getImageData(0,0,this.width,this.height);for(let i=0;i<d.data.length;i+=4){d.data[i]=Math.min(255,Math.max(0,d.data[i]+noise(i)));d.data[i+1]=Math.min(255,Math.max(0,d.data[i+1]+noise(i+1)));d.data[i+2]=Math.min(255,Math.max(0,d.data[i+2]+noise(i+2)));}ctx.putImageData(d,0,0);}
      return orig.apply(this,a);
    };
  }catch(e){}
  try{
    const orig=AudioBuffer.prototype.getChannelData;
    AudioBuffer.prototype.getChannelData=function(ch){const d=orig.call(this,ch);for(let i=0;i<d.length;i++)d[i]+=noise(i)*0.0001;return d;};
  }catch(e){}
  try{
    const orig=WebGLRenderingContext.prototype.readPixels;
    WebGLRenderingContext.prototype.readPixels=function(...a){orig.apply(this,a);const p=a[6];if(p instanceof Uint8Array){for(let i=0;i<p.length;i+=4){p[i]=Math.min(255,Math.max(0,p[i]+noise(i)));p[i+1]=Math.min(255,Math.max(0,p[i+1]+noise(i+1)));p[i+2]=Math.min(255,Math.max(0,p[i+2]+noise(i+2)));}}};
  }catch(e){}
})();`;
}
