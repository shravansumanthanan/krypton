...
process.on('uncaughtException', (error) => {
    console.error('[KryptonBrowser] Uncaught Exception:', error);
});
process.on('unhandledRejection', (reason, promise) => {
    console.error('[KryptonBrowser] Unhandled Rejection at:', promise, 'reason:', reason);
});
...
function loadBlocklist() {
    try {
        ...
        const categories = ['ads', 'trackers', 'fingerprinting', 'cryptominers',
            'malware', 'social_tracking', 'telemetry'];
        ...
        if (Array.isArray(data.url_patterns)) {
            BLOCK_PATTERNS = data.url_patterns.map(p => {
                try { return new RegExp(p, 'i'); } catch { return null; }
            }).filter(Boolean);
        }

        console.log(`[KryptonBrowser] Blocklist loaded: ${BLOCK_SET.size} domains, ${BLOCK_PATTERNS.length} patterns`);
    } catch (e) {
        console.error('[KryptonBrowser] Failed to load blocklist:', e.message);
    }
}
...
function recordSiteBlock(requestUrl, category) {
    try {
        ...
        if (!siteBlockCounts.has(hostname)) {
            ...
            siteBlockCounts.set(hostname, { total: 0, ads: 0, trackers: 0, fingerprinting: 0, cryptominers: 0, malware: 0, social: 0, telemetry: 0, patterns: 0 });
        }
...
function createWindow() {
    ...
    mainWindow.webContents.on('console-message', (event, level, message) => {
        ...
        if (level > 0) console.log(`[Renderer ${tags[level] || 'INFO'}] ${message}`);
    });
...
function setupRequestInterception(ses) {
    ...
    ses.webRequest.onBeforeRequest({ urls: ['*://*/*'] }, (details, callback) => {
        ...
        // HTTPS upgrade
        if (getConfig('krypton_https_upgrade', 'true') === 'true'
            && url.startsWith('http://')
    ...
    ses.webRequest.onBeforeSendHeaders({ urls: ['*://*/*'] }, (details, callback) => {
        ...
        if (headers['User-Agent']) {
            headers['User-Agent'] = headers['User-Agent'].replace(/\s*$/, ' KryptonBrowser/1.0 PQC-Enabled');
        }
    ...
    ses.setPermissionRequestHandler((wc, permission, callback, details) => {
        const denied = ['camera', 'microphone', 'geolocation', 'notifications',
            'midi', 'pointerLock', 'serial', 'bluetooth', 'hid', 'usb'];
        if (denied.includes(permission)) {
            if (mainWindow) mainWindow.webContents.send('permission-request', {
                permission,
...
function setConfig(key, value) {
    ...
    cfg[key] = value;
    _configCache = cfg;
    try { fs.writeFileSync(CONFIG_FILE, JSON.stringify(cfg, null, 2)); } catch (e) { console.error(e); }
}
...
const ALLOWED_CONFIG_KEYS = new Set([
    'krypton_ad_block', 'krypton_https_upgrade', 'krypton_send_dnt',
    'krypton_block_cookies', 'krypton_ask_download_loc',
]);
...
ipcMain.handle('get-site-block-count', async (e, pageUrl) => {
    ...
    try {
        ...
        return siteBlockCounts.get(hostname) || { total: 0, ads: 0, trackers: 0, fingerprinting: 0, cryptominers: 0, malware: 0, social: 0, telemetry: 0, patterns: 0 };
    } catch { return null; }
});
...
ipcMain.handle('get-certificate-info', async (e, url) => {
    try {
        ...
        return await new Promise(resolve => {
            ...
            req.on('response', res => { resolve({ statusCode: res.statusCode, url }); req.abort(); });
...
function setupDownloadManager() {
    session.defaultSession.on('will-download', (e, item) => {
        ...
        const info = {
            id, filename: fileName, url: item.getURL(),
            savePath: item.getSavePath(),
            totalBytes: item.getTotalBytes(),
            receivedBytes: 0, state: 'progressing',
            startTime: Date.now(), speed: 0,
        };
        downloads.push(info);
...
ipcMain.handle('open-download', async (e, p) => {
    ...
    try { shell.openPath(path.resolve(p)); } catch { }
});
ipcMain.handle('show-download-in-folder', async (e, p) => {
    if (!isValidDownloadPath(p)) return;
    try { shell.showItemInFolder(path.resolve(p)); } catch { }
});
ipcMain.handle('open-downloads-folder', async () => { try { shell.openPath(app.getPath('downloads')); } catch { } });
ipcMain.handle('choose-download-path', async () => {
    const r = await dialog.showOpenDialog(mainWindow, { properties: ['openDirectory'], title: 'Choose Download Location' });
    ...
    Menu.setApplicationMenu(Menu.buildFromTemplate([
        {
            ...
            submenu: [
                ...
                { type: 'separator' },
                { label: 'PQC Security Panel', accelerator: 'CmdOrCtrl+Shift+P', click: () => mainWindow?.webContents.send('navigate-to', 'krypton://pqc-security') },
                { type: 'separator' },
        ...
        },
        {
            ...
            submenu: [
                ...
                { label: 'New Private Window', accelerator: 'CmdOrCtrl+Shift+N', click: () => mainWindow?.webContents.send('menu-action', 'private-window') },
                { label: 'Close Tab', accelerator: 'CmdOrCtrl+W', click: () => mainWindow?.webContents.send('close-tab') },
                { type: 'separator' },
                { label: 'Open Location', accelerator: 'CmdOrCtrl+L', click: () => mainWindow?.webContents.send('focus-url-bar') },
                { type: 'separator' },
                { label: 'Find in Page', accelerator: 'CmdOrCtrl+F', click: () => mainWindow?.webContents.send('toggle-find-bar') },
            ],
        },
        {
            label: 'Edit',
        ...
        },
        {
            label: 'View',
            submenu: [
                { label: 'Reload', accelerator: 'CmdOrCtrl+R', click: () => mainWindow?.webContents.send('reload-page') },
                { type: 'separator' },
                { label: 'Toggle Sidebar', accelerator: 'CmdOrCtrl+B', click: () => mainWindow?.webContents.send('menu-action', 'toggle-sidebar') },
                { label: 'Reader Mode', accelerator: 'CmdOrCtrl+Shift+R', click: () => mainWindow?.webContents.send('menu-action', 'reader-mode') },
                { label: 'Shields Panel', accelerator: 'CmdOrCtrl+Shift+S', click: () => mainWindow?.webContents.send('menu-action', 'toggle-shields') },
                { type: 'separator' },
                { role: 'toggleDevTools' },
                { type: 'separator' },
        ...
        },
        {
            ...
            submenu: [
                { label: 'PQC Security Panel', accelerator: 'CmdOrCtrl+Shift+P', click: () => mainWindow?.webContents.send('navigate-to', 'krypton://pqc-security') },
                { type: 'separator' },
                { label: 'View Connection Security', click: () => mainWindow?.webContents.send('show-security-info') },
                { type: 'separator' },
...
process.on('uncaughtException', (err) => {
    console.error('[KryptonBrowser] CRITICAL: Uncaught Exception:', err);
    // In a real Army deployment, we would log this securely to a centralized logging facility
});

process.on('unhandledRejection', (reason, promise) => {
    console.error('[KryptonBrowser] CRITICAL: Unhandled Rejection at:', promise, 'reason:', reason);
});
